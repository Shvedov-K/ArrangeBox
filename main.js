...
(function(){
    ...
    const descending = document.querySelector('.js-sort-dc');
    descending.addEventListener("click", sortContainers);

    const ascending = document.querySelector('.js-sort-asc');
    ...
    initButton.addEventListener("click", initValue);

    const randButton = document.querySelector('.js-rand-button');
    randButton.addEventListener("click", randValue);
    ...
    function randValue() {
        ...
        for (var i = 0; i < count; i++) {
            ...
            container1.append(createElement(Math.floor(Math.random() * 100)));
        }
    }
    ...
    function sortContainers(event) {
        const target = event.target;
        if (target.classList.contains("js-sort-dc")) {
            sortContainer(container1, true);
            sortContainer(container2, true);
        } else if (target.classList.contains("js-sort-asc")) {
